...
export default function ProductCategoriesPage() {
    ...
    const debouncedSearch = useCallback(
            ...
            return async (searchValue: string) => {
                ...
                timeoutId = setTimeout(async () => {
                    if (searchValue.trim()) {
                        setLoading(true)
                        try {
                            const response = await apiClient.searchProductCategories(searchValue, {
                                page: 1,
                                per_page: perPage,
                                sort_by: sortState.sortBy,
                                sort_order: sortState.sortOrder
        ...
        [perPage, sortState, loadCategories]
    )
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.productCategoryName.trim()) {
            errors.productCategoryName = 'Category name is required'
        } else if (formData.productCategoryName.trim().length > 100) {
            errors.productCategoryName = 'Category name must be less than 100 characters'
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingCategory) {
                response = await apiClient.updateProductCategory(editingCategory.productCategoryId, formData)
            } else {
                response = await apiClient.addNewProductCategory(formData)
            }
            ...
            if (response.success) {
                ...
                setFormErrors({ general: response.message || 'Failed to save category' })
            }
        } catch (error) {
    ...
    const handleDeleteCategory = async () => {
        ...
        setIsDeleting(true)
        try {
            const response = await apiClient.deleteProductCategory(deletingCategory.productCategoryId)
    ...
    const columns = useMemo(() => [
        {
            name: 'Category',
            selector: (row: ProductCategory) => row.productCategoryName,
            sortable: true,
            ...
            cell: (row: ProductCategory) => (
                <div className="flex space-x-2">
                    <button
                        ...
                        aria-label={`Edit category ${row.productCategoryName}`}
                    >
                    ...
                    <button
                        ...
                        aria-label={`Delete category ${row.productCategoryName}`}
                    >
    ...
    const handleExportToCSV = async () => {
        ...
        try {
            ...
            const csvData = allCategories.map(category => ({
                'Category ID': category.productCategoryId,
            ...
            toast.success(`Successfully exported ${allCategories.length} categories to CSV`)
        } catch (error) {
    ...
    const handleExportToExcel = async () => {
        ...
        try {
            ...
            toast.success(`Successfully exported ${allCategories.length} categories to Excel`)
        } catch (error) {
            toast.error('Failed to export categories. Please try again.')
        } finally {
            setIsExporting(false)
                        ...
                        <div className="card-header">
                            <div className="flex justify-between items-center">
                                <h3 className="text-lg font-medium text-gray-900">
                                    Categories {totalRows > 0 ? `(${totalRows})` : ''}
                                </h3>
                            ...
                            <DataTable
                                ...
                                progressComponent={
                                    <div className="flex items-center justify-center py-12">
                                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600" aria-label="Loading categories"></div>
                                    </div>
                                }
                            />
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className={`input w-full ${formErrors.productCategoryName ? 'border-red-500' : ''}`}
                                    required
                                />
                                {formErrors.productCategoryName && (
                                    <p className="text-red-500 text-sm">{formErrors.productCategoryName}</p>
                                )}
                            </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    {isSubmitting ? 'Adding...' : (editingCategory ? 'Update Category' : 'Add Category')}
                                </button>
                                <button
                                    onClick={closeModal}
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-900">
                                        {deletingCategory.productCategoryName}
                                    </p>
                                </div>
                            </div>
